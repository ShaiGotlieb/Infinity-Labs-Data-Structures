...
{
	dll_iterator_t src_iterator; 
	dll_iterator_t src_runner_iterator;
	dll_iterator_t dest_iterator;

	assert(NULL != list_dest);
	assert(NULL != list_src);

	src_iterator = DLLBegin(list_src->dllist); 
	src_runner_iterator = DLLBegin(list_src->dllist);
	dest_iterator = DLLBegin(list_dest->dllist);

	if (DLLIsEmpty(list_src->dllist))
	{
		return;
	}

	if (DLLIsEmpty(list_dest->dllist))
	{
